...
/**
 * Classify open events - Simplified to treat all opens as human/actual opens
 */
function classifyOpenEvents(events: CampaignEvent[]): Map<string, 'human'> {
    const classifications = new Map<string, 'human'>();
    events.forEach(event => {
        if (event.type === 'open') {
            classifications.set(event.id, 'human');
        }
    });
